{-
 ...
 -}
module CALL_BY_NAME.Interp (interp,runInterp,prelude,Answer) where
...
import           CALL_BY_NAME.Closure
import           CALL_BY_NAME.Environment
import           CALL_BY_NAME.Pair
import           CALL_BY_NAME.Parser
import           CALL_BY_NAME.Prelude
...
runInterp = do
    args <- getArgs
    if null args
        then putStrLn "CALL_BY_NAME: Missing source file name"
...
valueOf (VarExp x) ρ σ = valueOfThunk (deref addr σ) σ
  where
    addr = applyEnv ρ x
